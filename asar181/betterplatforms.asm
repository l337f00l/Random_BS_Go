...
LDA !9E,x
CMP #$B1        ;dont screw with the eating block
BEQ .DontCheck2
CMP #$79        ;or the growing vine
BEQ .DontCheck2
CMP #$45        ;or the directional coins
BEQ .DontCheck2
CMP #$1F        ;or kameks
BEQ .DontCheck2
CMP #$20        ;or its magic
...
;climbing koopa set
CMP #$22        ;below climbing koopa set its all fine
BCC .DoCheck
CMP #$26        ;above climbing koopa set its all fine
BCC .DontCheck2
...
;dont check bottom or sides if the sprite is a raft piece
LDA !9E,x
CMP #$61
BNE +
JMP .DontCheck
...
LDA !9E|!dp,y
CMP #$49        ;growing/shrinking pipe
BEQ .YStand
CMP #$57        ;vertical checkerboard
BEQ .YStand
CMP #$58        ;vertical rock
BEQ .YStand
CMP #$8F        ;shroom scales
BEQ .YStand
CMP #$C4        ;grey falling platform
BEQ .YStand
...
BrownFix:
PHY
JSL $03B72B|!bank ;check if mario and the sprite interact
PHP         ;keep the result of mario<->sprite interaction safe
...
;restore mario interaction result and the rest of our hijack
PLP
PLY
STZ $1602|!addr,x
RTL
...
if !SpriteSpring == $01
  ...
  ;catapult certain sprites slightly less
  LDA !9E|!dp,y
  CMP #$4F                  ;piranha plant
  BEQ .LittleBounce
  CMP #$50                  ;piranha plant
  BEQ .LittleBounce
  CMP #$86                  ;wiggler
  BEQ .LittleBounce
  ...
  LDA !9E|!dp,y
  CMP #$B0          ;dont spring up boo streams
  BEQ .Restore
  CMP #$B6          ;or reflecting fireballs
  BEQ .Restore
  ...
  ;give a few sprites a slightly different interaction
  LDA !9E|!dp,y
  CMP #$4F
  BEQ .LittleChange
  CMP #$50
  ...
  TYX
  LDA !SpriteBlockTable,x
  AND #$04^$FF            ;tell the sprite that it actually isn't on any floor at all
  STA !SpriteBlockTable,x
  PLX
...
if !SpritePeas == $01
  ...
  LDA !9E|!dp,y
  CMP #$B0        ;dont bounce boo streams
  BEQ .RestorePLY
  CMP #$B6        ;or reflecting fireballs
  BEQ .RestorePLY
