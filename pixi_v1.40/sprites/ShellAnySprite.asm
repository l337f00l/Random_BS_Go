...
; Shell Any-Sprite
; By: Bucket
;     Shell sprite by Kevin.
;	EXTRA BYTE 1: sprite number
;	EXTRA BYTE 2: X offset
;   EXTRA BYTE 3: 0 or 1. 0 means X offset will push right.
;   EXTRA BYTE 4: 0 or 1. 0 means stationary state. 1 means Kicked.
; Use the offsets to ensure the object is carriable
; from one side or the other.
;----------------------------------------------

!Num_No_Contact_Frames_On_Carry = #$16
!Address_Sprite_Lock_Flag = $9D


; NOTE: The shell sprite uses all available sprite table data addresses
; aside from 1FD6. So we're going to use:
;  0F5E - which is some freeram.
;  0E25 - which is an overworld sprite table.
;  0E15 - which is an overworld sprite table.
!Sprite_Table_Spawned_Sprite_Index = $1FD6
!Sprite_Table_Carried_Direction = $0F5E
!Sprite_Table_X_Offset = $0E25
!Sprite_Table_Sprite_Number = $0E15

!Sprite_Table_Y_Position_Low = $D8
!Sprite_Table_X_Position_Low = $E4
!Sprite_Table_Y_Position_High = $14D4
!Sprite_Table_X_Position_High = $14E0
...
print "INIT ",pc
    ...
    ; so here we just want to load in the sprite data
    ; and spawn it. we then want to store our spawned-sprite-index
    ; into freeram so we can reference it later.
    ...
    LDA !extra_byte_2,x
    STA !Sprite_Table_X_Offset,x

    ; sprite number is stored in the Y register.
    ; so we are going to index our freeram by the main's sprite's
    ; index, and store our new sprite's index there.
    ; nice and clear as mud.
    TYA
    STA !Sprite_Table_Spawned_Sprite_Index,x
...
.left
    LDA #$00
.dirset
    STA !Sprite_Table_Carried_Direction,x

    ; set the 'dont interact with other sprites' flag
    LDA $1686,y
    ORA #$08
    STA $1686,y
    PLY
    ...
    LDA !extra_byte_1,x
    STA !Sprite_Table_Sprite_Number,x

    ; now we gotta set our extra bytes to manipulate the 
    ; code for the koopashell by Kevin.
    ; "Name": "Green Shell (stationary)",
    ; "ExtraBit": true,
    ; "Extra Property Byte 1": 64,
    ...
    ; 2E if we want to spawn in a kicked state
    LDA !extra_byte_4,x
    BEQ .set3
    LDA #$2E
.set3
    ...
    ; and finally call the koopashell init.
    JSL shell_init
...
sprite_main:
    ...
    ; just quick the sprite lock status first
    LDA !Address_Sprite_Lock_Flag
    BNE .skiptoreturn

    ; kill ourselves if we're offscreen
    LDA #$00
    %SubOffScreen()

    ; check really quick if we went off screen.  If so, we'll
    ; delete the sprite as well.
    LDA $14C8,x
    CMP #$07
    BPL .continue
    ; we went offscreen, so we copy our status over to
    ; the other sprite.
    PHY
    LDY !Sprite_Table_Spawned_Sprite_Index,x
    LDA $14C8,x
    STA $14C8,y
    PLY
    BRA .skiptoreturn
...
.continue
    ...
    ; do regular shell stuff
    JSL shell_main

    LDY !Sprite_Table_Spawned_Sprite_Index,x

    ; check the status of the spawned sprite. if it's done, we'll leave too.
    LDA $14C8,y
    CMP #$07
    BPL .alive

    ; our sprite died, so we'll remove ourselves as well
    ; and and kick out to the return.
    LDA #$00
    STA $14C8,x
...
.alive
    ; copy the shell's position over to the spawned
    ; sprite's position.

    LDA !Sprite_Table_X_Position_High,x
    STA !Sprite_Table_X_Position_High,y
    LDA !Sprite_Table_X_Position_Low,x
    STA !Sprite_Table_X_Position_Low,y
    LDA !Sprite_Table_Y_Position_High,x
    STA !Sprite_Table_Y_Position_High,y
    LDA !Sprite_Table_Y_Position_Low,x
    STA !Sprite_Table_Y_Position_Low,y

    LDA #$00
    STA !Sprite_Table_X_Speed,y
    STA !Sprite_Table_Y_Speed,y

    ; and now we apply our offsets.
    ; first check the 'carried' status.
    PHY
    ...
    BEQ .carriedoffsets
    CMP #$0A
    ; kicked state - no offset
    BEQ .afterxoffset
    ; stationary, dropped, etc.
    BRA .directionaloffsets

.directionaloffsets
    ; for kicked state, we just want to keep
    ; the same offset as the most recent left/right state.
    LDA !Sprite_Table_Carried_Direction,x
    BNE .add
    BRA .subtract
...
.subtract
    ; subtract the offset if we're not in a kicked state.
    LDA !Sprite_Table_X_Offset,x
    JSR subtract_from_sprite_position_x
    BRA .afterxoffset
.add
    ; add the offset if we're not in a kicked state
    LDA !Sprite_Table_X_Offset,x
    JSR add_to_sprite_position_x
.afterxoffset

    ; now we're handling negative Y offsets.
    ; we're just kinda hard-coding shit in here.
    ; because we're good devs.
    LDA !Sprite_Table_Sprite_Number,x
    ; sprite: big-yoshi
    CMP #$35
    BEQ .onetileup
    ; sprite: hot-head
...
.onetileup
    ...
    BRA .applyyoffset
.applyyoffset
    JSR subtract_from_sprite_position_y

    ; now we need to handle mario interaction with the spawned sprite.
    ; for example - we don't want to interact with the spawned sprite
    ; if we're carrying the invisi-shell.
    LDA $14C8,x
    CMP #$0B
    BEQ .nointeraction
    CMP #$0A
    BEQ .nointeractionwithshell
    PLY
...
.nointeraction
    PLY
    LDA !Num_No_Contact_Frames_On_Carry
    STA $154C,y
    BRA .doneinteraction
.nointeractionwithshell
    PLY
    LDA !Num_No_Contact_Frames_On_Carry
    STA $154C,x
    BRA .doneinteraction
...
;------------------- Add to Position. --------------------------
; Utility Method: Adds to Y Position
; input: A = Y Offset
...
;------------------- Add to Position. --------------------------
; Utility Method: Adds to X Position
; input: A = X Offset
...
; input: A = X Offset
subtract_from_sprite_position_x:
    ; this is nice and funkey. we're going to use 16-bit math here.
    ; store value in 04
    STA $04
    ; store value in 05
    ; (this is 16 bit math, so we need to zero our our "HIGH BYTE" here.)
    ; (also the high byte is stored in the 2nd position because it's little endian.)
    STZ $05
    ; load high byte
    LDA $14E0,y
    ; swap this over to the high-byte A
    XBA
    ; load the low byte
    LDA $E4,y
...
;------------------- Subtract From Position. --------------------------
; input: A = Y Offset
subtract_from_sprite_position_y:
    ; this is nice and funkey. we're going to use 16-bit math here.
    ; store value in 04
    STA $04
    ; store value in 05
...
SpinJumpKill:
    ; Disallowing Spin-Jump Interaction here.
    RTS

               ;> Otherwise, kill with a spinjump.
    jsl $01AB99|!bank       ;> Display contact GFX
...
+   ora $64                 ;|
    ORA #$01                ;| Using Page 4 instead of 3.
    sta $0303|!addr,y       ;/
    tya                     ;\ Set size (16x16).
    lsr #2                  ;|
    tay                     ;|
